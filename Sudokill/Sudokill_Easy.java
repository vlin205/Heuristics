...
public class Sudokill_Easy {
	...
	public static void main(String[] args) throws Exception {
	    ...
	    while ((fromServer = in.readLine()) != null) {
	        ...
	        else if (fromServer.equals("ADD")) {
				Move temp = updateBoard(fromServer);
				out.println(temp.toString() + " " + playerNumber);
	        }
	...
	public static void updateLastMove(String str) {
		...
		if((int)inputString.get(3) != playerNumber) {
	...
	public static ArrayList<Move> findValidMoves(State state) {
		...
		for(int i = 0; i < 9; i++) { //checks the columns of the row
			if(state.getBoard()[row][i] == 0) {
				for(int j = 1; j < 10; j++) {//checks the 1-9 values
					if((state.getRows()[row] & (1<<j)) == 0 && (state.getColumns()[i] & (1<<j)) == 0 && (state.getBlocks()[((i/3) + (row/3)*3)] & (1<<j)) == 0) {
		...
		for(int i = 0; i < 9; i++) {
			if(state.getBoard()[i][column] == 0) {
				for(int j = 1; j < 10; j++) {
					if((state.getRows()[i] & (1<<j)) == 0 && (state.getColumns()[column] & (1<<j)) == 0 && (state.getBlocks()[((column/3) + (i/3)*3)] & (1<<j)) == 0) {
